...
export class SessionRecovery {
  ...
  async recoverSession(): Promise<Session | null> {
    ...
    if (this.recoveryPromise) {
      ...
      if (this.recoveryStartTime && now - this.recoveryStartTime > 10000) {
        ...
        console.log(
          "[SessionRecovery] Recovery already in progress, returning existing promise",
        );
  ...
  private async performRecovery(): Promise<Session | null> {
    try {
      console.log(
        "[SessionRecovery] Attempting to recover session from storage...",
      );
      ...
      if (session) {
        ...
        console.log(`[SessionRecovery] session: ${session}`);
        ...
        if (userError || !user) {
          console.log(
            "[SessionRecovery] Session invalid, attempting refresh...",
          );
          ...
          if (!refreshError && refreshedSession) {
            ...
            console.error(
              "[SessionRecovery] Failed to refresh session:",
              refreshError,
            );
            return null;
          }
      ...
      console.log(
        "[SessionRecovery] No session from getSession, checking localStorage directly...",
      );
      ...
      if (storedData) {
        try {
          ...
          if (parsed?.currentSession?.refresh_token) {
            console.log(
              "[SessionRecovery] Found refresh token in localStorage, attempting to restore...",
            );
            ...
            if (!restoreError && restoredSession) {
              console.log(
                "[SessionRecovery] Session restored from localStorage",
              );
              return restoredSession;
            } else {
              console.error(
                "[SessionRecovery] Failed to restore session:",
                restoreError,
              );
            }
          }
