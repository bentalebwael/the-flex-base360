...
interface RevenueData {
    property_id: string;
    total_revenue: string; // Changed to string to handle high precision financial data
    currency: string;
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    // Use tenant_id from auth context, fallback to debugTenant or candidate
    const activeTenant = debugTenant || user?.tenant_id || 'candidate';
    ...
    // Use parseFloat only for display. Since total_revenue is now a string, 
    // we avoid intermediate floating point errors during data transfer.
    const revenueValue = parseFloat(data.total_revenue);
