...
export function useCities(options: UseCitiesOptions = {}): UseCitiesResult {
  ...
  const fetchCities = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "[useCities] result.cities is array:",
        Array.isArray(result?.cities),
      );
      console.log(
        "[useCities] result.cities length:",
        result?.cities?.length || "undefined",
      );
      ...
      if (cities.length > 0) {
        ...
        console.log(
          "[useCities] 🎯 Final parsed cities (city-access API):",
          parsed,
        );
      } else {
      ...
      console.log(
        "[useCities] ✅ Setting cities in state:",
        parsed.length,
        "cities",
      );
      setCities(parsed);
    ...
    cacheKey,
    options.userAccessibleOnly,
    options.includePropertyCount,
    authLoading,
    user,
  ]);
...
export function getCityDisplayInfo(
  cityId: string,
  context?: "default" | "cleaning",
) {
  const cityMap: Record<
    string,
    { name: string; icon: string; image: string; cleaningImage?: string }
  > = {
    london: {
      name: "London",
      icon: "Landmark",
    ...
    algiers: {
      ...
      icon: "MapPin",
      image:
        "https://images.unsplash.com/photo-1627221889894-c9cfa8e843bf?q=80&w=3174&auto=format&fit=crop",
    },
    lisbon: {
      name: "Lisbon",
      icon: "MapPin",
      image:
        "https://images.unsplash.com/photo-1545208942-d4c8c7e3e1e2?auto=format&fit=crop&w=1920&q=80",
      cleaningImage:
        "https://cdn2.civitatis.com/portugal/lisboa/guia/lisbon-tram-seo.jpg",
    },
  };
