...
async def calculate_monthly_revenue(property_id: str, month: int, year: int, property_timezone: str = 'UTC', db_session=None) -> Decimal:
    """
    Calculates revenue for a specific month, considering property timezone.
    
    FIXED: Convert dates to property's local timezone to ensure reservations
    are counted in the correct month according to the property's location.
    """
    # Get timezone object
    ...
    print(f"DEBUG: Querying revenue for {property_id} from {start_date} to {end_date} (timezone: {property_timezone})")
